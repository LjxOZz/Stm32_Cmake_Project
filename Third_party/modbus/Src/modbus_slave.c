...
*********************************************************************************************************
*/
void MODS_ReciveNew(uint8_t *Data, uint16_t Size)
{
	/*
	...
	*/
	...
	bsp_StartHardTimer(ModbusBaudRate[i].usTimeOut, (void *)MODS_RxTimeOut);

	if (Size < S_RX_BUF_SIZE)
	{
		g_tModS.RxCount = Size;
		memcpy(g_tModS.RxBuf, Data, Size);
	}
}
...
static void MODS_AnalyzeApp(void)
{
	Rs485_Send(g_tModS.RxBuf, g_tModS.RxCount);
